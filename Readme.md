...
`new` は現在のモードを保ったまま、会話履歴とメモリーをリセットして新しいセクションに移ります。`.agent_sandbox/` 内の生成ファイルは削除しません。
...
`調べて要約して` のような調査依頼では、まずGoogleでキーワード検索し、上位10件のリンク先に移動して本文を読み込みます。各ページでは要約せず本文をコピーし、最後の統合時だけLLMで1パラグラフ程度へ要約します。
